...
//２．データ登録SQL作成
$stmt = $pdo->prepare("SELECT * FROM gs_bm_table WHERE user_id=$jibun");
$status = $stmt->execute();
...
if($status==false){
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
      $view .= '<div style="background-color:#eca ; width:400px">';
      $view .= '<img width="100%" style="display:block"  src="upload/'.$result["image"].'">';
      $view .= '</div>';
      $view .= '<div style="background-color:#c84 ; width:400px;height:30px ; font-size:10pt ;display:flex ;justify-content: space-between;align-items: center ; margin-bottom:10px" >';
...
<!-- Main[Start] -->
   ...
   <legend style="font-size:16pt">My art　</legend>
   <input type="button" value="データ登録" onClick="location.href='inform.php'">　
   <input type="button" value="ユーザー編集" onClick="location.href='user_detail.php'">
